...
#trim adapters
for i in `ls -1 *.fastq.gz | sed 's/.fastq.gz//'`; do
	trimmomatic SE -phred33 -threads $n ${i}.fastq.gz ${i}.trim.fastq.gz ILLUMINACLIP:/home/rcf-proj/zl1/adapter/TruSeq3-SE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36 &> ${i}.trim.txt
done
...
#reads alignment
cd trim
for i in `ls -1 *.trim.fastq.gz | sed 's/.trim.fastq.gz//'`; do
